...
```css
.mode-toggle { display: flex; gap: 0; margin-bottom: 1.25rem; }
.mode-toggle button { flex: 1; border: 1px solid var(--border); background: var(--btn-bg); color: var(--text); cursor: pointer; }
...
.mode-toggle button:not(.active):hover { background: var(--btn-hover); }
```
